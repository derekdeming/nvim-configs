...
# Local environment and PATH additions
...
if [[ -x "$(command -v lsd)" ]]; then
	alias ls='lsd -F --group-dirs first --all'
...
# auto-unalias potential conflicting g* shortcuts (added by fix)
for a in gcm gco gbr gaa gst gst_full gpl gps glg gundo; do
...
gcm() {
  if [[ "${1:-}" == "--clip" || "${1:-}" == "-c" ]]; then
    if command -v pbpaste >/dev/null 2>&1; then
      pbpaste | command git commit -F -
    elif command -v xclip >/dev/null 2>&1; then
      xclip -o | command git commit -F -
    elif command -v wl-paste >/dev/null 2>&1; then
      wl-paste -p | command git commit -F -
    else
      echo "No clipboard helper found (pbpaste/xclip/wl-paste). Use paste+Ctrl-D fallback."
      return 1
    fi
    return $?
  fi
  ...
  if (( $# > 0 )); then
    if [[ "$1" == -* ]]; then
      noglob command git commit "$@"
    else
      noglob command git commit -m "$*"
    fi
    return $?
  fi

  echo "Paste your commit message, then press Ctrl-D on an empty line to finish."
  command git commit -F -
}

gco()       { noglob command git checkout "$@"; }
gbr()       { noglob command git branch "$@"; }
gaa()       { noglob command git add --all "$@"; }
gst()       { noglob command git status -sb "$@"; }
gst_full()  { noglob command git status "$@"; }
gpl()       { noglob command git pull --rebase --autostash "$@"; }
gps()       { noglob command git push -u origin HEAD "$@"; }
glg()       { noglob command git log --graph --oneline --decorate "$@"; }
gundo()     { noglob command git reset --soft HEAD^ "$@"; }
...
#######################################################
if [[ -f "$HOME/.config/zsh/zsh-syntax-highlightin-tokyonight.zsh" ]]; then
	source "$HOME/.config/zsh/zsh-syntax-highlightin-tokyonight.zsh"
fi
